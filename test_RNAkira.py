...
for output,results,statsmodel in zip([output_gaussian,output_nbinom],[results_gaussian,results_nbinom],['gaussian','nbinom']):
	...
	title='{0} genes, {1} time points, {2} replicates, {3} model\n{4}'.format(nGenes,len(time_points),len(replicates),statsmodel,stats)
	...
	fig.suptitle('{0} genes, {1} time points, {2} replicates, {3} model'.format(nGenes,len(time_points),len(replicates),statsmodel),size=10)
